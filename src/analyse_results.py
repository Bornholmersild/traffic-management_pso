...
def plot_metrics_from_multiple_csvs(base_path, save_path, num_files):
    ...
    a, b = fit_exponential(fitness_data['Iteration'], fitness_data['Mean'])
    print(f"Run {i}: Fitness ~ {a:.3f} * exp({b:.3f} * Iteration)")
    ...
    for i in range(1, num_files + 1):
        ...
        ax2.plot(non_arrived_data['Iteration'], non_arrived_data['Mean'], label=f'Non-Arrived Vehicles (Run {i})')
    ...
    combined_save_path = os.path.join(save_path, "Arrived_nonArrived_vehicles.svg")
    plt.tight_layout()
    ...
    for i in range(1, num_files + 1):
        ...
        ax1.plot(trip_time_data['Iteration'], trip_time_data['Mean'], label=f'Total Trip Time (Run {i})')
    ...
    combined_save_path = os.path.join(save_path, "trip_wait_time.svg")
    plt.tight_layout()
    plt.savefig(combined_save_path)
    plt.close()
...
def plot_hist_for_metrics(base_paths, num_files, save_path="output"):
    metrics = ['fitness', 'arrived_vehicles', 'non_arrived_vehicles', 'total_trip_time', 'total_wait_time']
    ...
    for idx, metric in enumerate(metrics):
        ...
        ax.set_xticklabels([f'Path {i+1}' for i in range(len(base_paths))], fontsize=20)
        ax.set_title(f'{metric.capitalize()}', fontsize=24)
        ax.set_ylabel('Mean Value', fontsize=20)

    plt.tight_layout()
    save_path = os.path.join(save_path, "grid_world_last_mean_results.svg")
    plt.savefig(save_path)
    plt.close()
    plt.show()
    

#plot_metrics_from_csv("output/line_10_tls_veh_200/logging_run1.csv", "output/line_10_tls_veh_200")
#plot_metrics_from_multiple_csvs("output/line_1_tls_200_veh/logging_run", "output/line_1_tls_200_veh", 5)
#plot_metrics_from_csv("output/line_1_tls_200_veh/logging_run4.csv", "output/line_1_tls_200_veh")
#plot_metrics_from_multiple_csvs("output/cross_1_tls_4_phases_400_veh/logging_run", "output/cross_1_tls_4_phases_400_veh",5)
